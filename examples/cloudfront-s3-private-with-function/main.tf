...
# ---------------------------------------------------------------------------------------------------------------------
# CREATE THE DEFAULT DIRECTORY INDEX CLOUDFRONT FUNCTION
# ---------------------------------------------------------------------------------------------------------------------

resource "aws_cloudfront_function" "default_directory_index" {
  name    = "${replace(module.static_website.website_bucket_name, ".", "-")}-default-directory-index"
  runtime = "cloudfront-js-1.0"
  comment = "CloudFront function to implement default directory indexing for private S3 bucket."
  publish = true
  code    = file("${path.module}/function/index.js")
}
...
module "cloudfront" {
  ...
  default_function_associations = [{
    ...
    function_arn = aws_cloudfront_function.default_directory_index.arn
  }]
