...
variable "failover_website_domain_names" {
  description = "The name of the website and the S3 bucket to create for failover (e.g. static-failover.foo.com)."
  type        = list(string)
}
...
variable "hosted_zone_id" {
  description = "The ID of the Route 53 Hosted Zone in which to create the DNS A Record specified in var.domain_name. Only used if var.create_route53_entries is true. Set to blank otherwise."
  type        = string
}

variable "acm_certificate_domain_name" {
  description = "The domain name for which an ACM cert has been issues (e.g. *.foo.com).  Only used if var.create_route53_entries is true. Set to blank otherwise."
  type        = string
}
