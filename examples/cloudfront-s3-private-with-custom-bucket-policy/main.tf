...
module "static_website" {
  ...
  # force object ownership to bucket owner
  s3_bucket_object_ownership = "BucketOwnerEnforced"

  # Add extra IAM Policies to the S3 bucket.
  s3_bucket_override_policy_documents = [for iam_policy in local.s3_bucket_policies : jsonencode(iam_policy)]
...
locals {
  s3_bucket_policies = yamldecode(templatefile(
    "${path.module}/s3-bucket-policies.yaml",
    {
